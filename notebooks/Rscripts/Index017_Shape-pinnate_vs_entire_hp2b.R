...
data_pre<-na.omit(object= dataall, cols=c('CHELSA_ai_stand', 'CHELSA_bio1_stand', 'CHELSA_bio12_stand', 'CHELSA_bio15_stand', 'Max_Rachis_Length_m_stand', 'HeightOverCanopy_stand'))
filter<-c("pinnate","entire")
data_hp2b<-subset(data_pre,shape %in% filter)
rownames(data_hp2b) <- data_hp2b$tip_name
data_hp2b$pinnate_binomial<-factor(data_hp2b$pinnate_binomial)
...
for(tree in 1:n_tree){
	...
	modelhp2b<- MCMCglmm(pinnate_binomial~CHELSA_ai_stand+CHELSA_bio1_stand+CHELSA_bio12_stand+CHELSA_bio15_stand+Max_Rachis_Length_m_stand+HeightOverCanopy_stand,
			random = ~animal,
			data = data_hp2b,
			reduced = TRUE,
			pedigree = tree2,
	...
	write.table(hp2b_postdist,"./Shape-pinnate_vs_entire_hp2b_postdist.txt",sep="\t")
}
write.table(hp2b_postdist,"./Shape-pinnate_vs_entire_hp2b_postdist.txt",sep="\t")
save.image("./Shape-pinnate_vs_entire_hp2b.Rimage")
