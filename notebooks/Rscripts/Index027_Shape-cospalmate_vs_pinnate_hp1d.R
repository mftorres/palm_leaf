...
data_hp1d<-filter(data_pre, cospalmate_binomial == "True" | pinnate_binomial == "True")
...
hp1d_postdist <- foreach(i=1:n_tree, .combine=rbind, .packages=packages) %dopar% {
	...
	write.table(hp1d_postdist,"./Shape-cospalmate_vs_pinnate_hp1d_postdist-2.txt",sep="\t")
}
write.table(hp1d_postdist,"./Shape-cospalmate_vs_pinnate_hp1d_postdist-2.txt",sep="\t")
save.image("./Shape-cospalmate_vs_pinnate_hp1d-2.Rimage")
