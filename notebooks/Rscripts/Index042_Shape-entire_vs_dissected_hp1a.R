...
hp1a_postdist <- foreach(i=1:n_tree, .combine=rbind, .packages=packages) %dopar% {
  ...
  modelhp1a<- MCMCglmm(dissection~CHELSA_ai_stand+CHELSA_bio1_stand+CHELSA_bio4_stand+CHELSA_bio15_stand+StemHeightBladeLength_stand,
                       random = ~animal,
                       data = data_hp1a,
                       reduced = TRUE,
                       pedigree = tree2,
  ...
  write.table(hp1a_postdist,"./Shape-entire_vs_dissected_hp1a_postdist-2.txt",sep="\t")
}
write.table(hp1a_postdist,"./Shape-entire_vs_dissected_hp1a_postdist-2.txt",sep="\t")
save.image("./Shape-entire_vs_dissected_hp1a-2.Rimage")
