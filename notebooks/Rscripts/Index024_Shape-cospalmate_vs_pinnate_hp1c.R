...
data_hp1c<-filter(data_pre, cospalmate_binomial == "True" | pinnate_binomial == "True")
rownames(data_hp1c) <- data_hp1c$tip_name
data_hp1c$cospalmate_binomial<-factor(data_hp1c$cospalmate_binomial)
...
hp1c_postdist <- foreach(i=1:n_tree, .combine=rbind, .packages=packages) %dopar% {
	...
	modelhp1c<- MCMCglmm(cospalmate_binomial~CHELSA_ai_stand+CHELSA_bio1_stand+CHELSA_bio4_stand+CHELSA_bio15_stand+HeightOverCanopy_stand,
			random = ~animal,
			data = data_hp1c,
			reduced = TRUE,
			pedigree = tree2,
...
write.table(hp1c_postdist,"./Shape-cospalmate_vs_pinnate_hp1c_postdist-2.txt",sep="\t")
save.image("./Shape-cospalmate_vs_pinnate_hp1c-2.Rimage")
