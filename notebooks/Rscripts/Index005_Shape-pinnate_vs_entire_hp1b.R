...
data_pre<-na.omit(object= dataall, cols=c('CHELSA_ai_stand', 'CHELSA_bio1_stand', 'CHELSA_bio12_stand', 'CHELSA_bio15_stand', 'StemHeightBladeLength_stand'))
filter<-c("pinnate","entire")
data_hp1b<-subset(data_pre,shape %in% filter)
rownames(data_hp1b) <- data_hp1b$tip_name
data_hp1b$pinnate_binomial<-factor(data_hp1b$pinnate_binomial)
...
for(tree in 1:n_tree){
	...
	modelhp1b<- MCMCglmm(pinnate_binomial~CHELSA_ai_stand+CHELSA_bio1_stand+CHELSA_bio12_stand+CHELSA_bio15_stand+StemHeightBladeLength_stand,
			random = ~animal,
			data = data_hp1b,
			reduced = TRUE,
			pedigree = tree2,
	...
	write.table(hp1b_postdist,"./Shape-pinnate_vs_entire_hp1b_postdist.txt",sep="\t")
}
write.table(hp1b_postdist,"./Shape-pinnate_vs_entire_hp1b_postdist.txt",sep="\t")
save.image("./Shape-pinnate_vs_entire_hp1b.Rimage")
