...
data_hp2a<-filter(data_pre, cospalmate_binomial == "True" | pinnate_binomial == "True")
...
hp1a_postdist <- foreach(i=1:n_tree, .combine=rbind, .packages=packages) %dopar% {
	...
	write.table(hp2a_postdist,"./Shape-cospalmate_vs_pinnate_hp2a_postdist-1.txt",sep="\t")
}
write.table(hp2a_postdist,"./Shape-cospalmate_vs_pinnate_hp2a_postdist-1.txt",sep="\t")
save.image("./Shape-cospalmate_vs_pinnate_hp2a-1.Rimage")
