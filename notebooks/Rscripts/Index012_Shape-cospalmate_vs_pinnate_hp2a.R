...
data_pre<-na.omit(object= dataall, cols=c('CHELSA_ai_stand', 'CHELSA_bio1_stand', 'CHELSA_bio4_stand', 'CHELSA_bio15_stand', 'Max_Rachis_Length_m_stand', 'HeightOverCanopy_stand'))
filter<-c("cospalmate","pinnate")
data_hp2a<-subset(data_pre,shape %in% filter)
...
for(tree in 1:n_tree){
	...
	write.table(hp2a_postdist,"./Shape-cospalmate_vs_pinnate_hp2a_postdist.txt",sep="\t")
}
write.table(hp2a_postdist,"./Shape-cospalmate_vs_pinnate_hp2a_postdist.txt",sep="\t")
save.image("./Shape-cospalmate_vs_pinnate_hp2a.Rimage")
