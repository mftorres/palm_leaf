...
data_hp1c<-filter(data_pre, entire_binomial == "True" | dissection == 1)
rownames(data_hp1c) <- data_hp1c$tip_name
data_hp1c$dissection<-factor(data_hp1c$dissection)
...
hp1c_postdist <- foreach(i=1:n_tree, .combine=rbind, .packages=packages) %dopar% {
	...
	modelhp1c<- MCMCglmm(dissection~CHELSA_ai_stand+CHELSA_bio1_stand+CHELSA_bio4_stand+CHELSA_bio15_stand+HeightOverCanopy_stand,
			random = ~animal,
			data = data_hp1c,
			reduced = TRUE,
			pedigree = tree2,
...
write.table(hp1c_postdist,"./Shape-entire_vs_dissected_hp1c_postdist-2.txt",sep="\t")
save.image("./Shape-entire_vs_dissected_hp1c-2.Rimage")
